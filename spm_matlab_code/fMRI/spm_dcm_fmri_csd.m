...
x = abs(10^-8*randn(size(x)));     % DHedit, introduce randomness to avoid singularities in automatic differentiation, we introduce it here to allow comparison between SPM and the Julia version
pE.A = pE.A .* (1 + 0.001*randn(size(pE.A)));
